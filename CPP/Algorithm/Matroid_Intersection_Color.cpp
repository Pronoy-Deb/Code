// Matroid Intersection Color Graphic Matroid:
...
struct GraphicOracle
{
    ...
    void prepare()
    {
        ...
        for (int i = 0; i < independent_set.size(); i++)
        {
            ...
            {
                if (i != j)
                    basis_without[i].add_edge(elements[independent_set[j]].edge);
            }
        }
    }
} graphic_oracle;
...
// Matroid Intersection Color Linear Matroid:
