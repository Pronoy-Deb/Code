...
vector<vector<mint>> Hessenberg(vector<vector<mint>> a)
{
    ...
    for (int col = 0, row = 0; col < n && row + 1 < n; ++col)
    {
        ...
        for (int i = 0; i < n; i++)
            swap(a[row + 1][i], a[cur][i]);
        for (int i = 0; i < n; i++)
            swap(a[i][row + 1], a[i][cur]); // applying a similar transformation so that determinant doesn't change
        ...
        for (int i = row + 2; i < n; i++)
        {
            if (a[i][col] != 0)
            {
                ...
                for (int j = 0; j < n; j++)
                    a[i][j] -= a[row + 1][j] * c;
                for (int j = 0; j < n; j++)
                    a[j][row + 1] += a[j][i] * c; // applying a similar transformation so that determinant doesn't change
            }
        }
        ++row;
    }
