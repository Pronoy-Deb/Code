...
{ // check if there is no articulation point
    low[u] = dis[u] = ++T;
    int child = 0;
    for (int v : g[u])
        if (v != pre)
...
{ // construct the dfs tree and preorder treversal
    low[u] = dis[u] = ++T;
    for (int v : g[u])
        if (v != pre)
        {
...
vector<int> st_numbering(int n, int s, int t)
{
    ...
    preorder.clear();
    for (int i = 1; i <= n; i++)
        dis[i] = low[i] = -1, sign[i] = 0;
    if (!dfs1(t))
        return vector<int>(); // no bipolar orientation if it is not 2-vertex connected after adding the s-t edge
    for (int i = 1; i <= n; i++)
        if (dis[i] == -1)
    ...
    {
        if (sign[low[v]] == -1)
            it[dis[v]] = lst.insert(it[dis[par[v]]], v);
        else
            it[dis[v]] = lst.insert(next(it[dis[par[v]]]), v);
