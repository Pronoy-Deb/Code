...
int32_t main()
{
    ...
    {
        if (a[i] < a[i + 1])
            add(i + 1, i, k, 0), add(i, i + 1, -1, 0);
        else if (a[i] > a[i + 1])
            add(i, i + 1, k, 0), add(i + 1, i, -1, 0);
        else
            add(i + 1, i, 0, 0), add(i, i + 1, 0, 0);
    }
    int s = n + 1;
    for (int i = 1; i <= n; i++)
        add(i, s, r, 0), add(s, i, -l, 0); // adding the constraint l <= a_i <= r
    memset(vis, 0, sizeof vis);
    for (int i = 1; i <= s; i++)
        d[i] = -1e9 - 9; // 1e9 + 9 for T = 0,  try to make INF as close to 0 as possible
    d[s] = 0;
    if (!spfa(s))
        puts("-1");
    else
